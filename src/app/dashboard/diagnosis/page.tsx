...
import { Upload, Microscope, Image as ImageIcon, CheckCircle, AlertTriangle, Clock, X, Leaf, RotateCcw } from 'lucide-react';
...
const SEVERITY_STYLES: Record<string, { cls: string; label: string }> = {
  low:      { cls: 'severity-low',      label: '🟢 Faible' },
  medium:   { cls: 'severity-medium',   label: '🟡 Moyen' },
  high:     { cls: 'severity-high',     label: '🟠 Élevé' },
  critical: { cls: 'severity-critical', label: '🔴 Critique' },
};
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const { data } = await api.post('/diagnosis', form, {
        ...
        timeout: 60000, // 60s — Gemini peut prendre du temps
      });
      // Réponse synchrone : diagnostic déjà complété
      setResult(data.diagnosis);
      setHistory((prev) => [data.diagnosis, ...prev]);
      toast.success('Diagnostic Gemini terminé ! 🌿');
    } catch (err: any) {
    ...
    <div className="fade-in" style={{ maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '2rem' }}>
        <div className="badge badge-green" style={{ marginBottom: '0.75rem', fontSize: '0.75rem' }}>
          <Microscope size={12} /> Gemini Vision IA
        </div>
        <h1 style={{ fontSize: '1.75rem', marginBottom: '0.35rem' }}>Diagnostic des Cultures</h1>
        <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>
          Photographiez votre plante — Gemini 1.5 Pro identifie la maladie et vous donne des recommandations
        </p>
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            <button id="analyze-btn" className="btn btn-primary" style={{ flex: 1 }}
              ...
              {uploading
                ? <><span className="spinner" /> Analyse Gemini en cours...</>
                : <><Microscope size={16} /> Analyser avec Gemini</>}
            </button>
            {preview && (
        ...
        <div>
          {polling && !result && (
            <div className="card glass" style={{ textAlign: 'center', padding: '3rem 1.5rem' }}>
              ...
              <p style={{ color: 'var(--green-400)', fontWeight: 600, marginBottom: '0.5rem' }}>Gemini analyse votre plante...</p>
            ...
            <div className="card glass fade-in" style={{ padding: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                  ...
                  {result.severity && (
                    <span className={`badge ${SEVERITY_STYLES[result.severity]?.cls}`}>
            ...
            {history.slice(0, 8).map((d) => (
              <div key={d._id} className="card glass" style={{ padding: '1rem', cursor: 'pointer', transition: 'all 0.3s' }} onClick={() => setResult(d)}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.5rem' }}>
                  ...
                  {d.severity && <span className={`badge ${SEVERITY_STYLES[d.severity]?.cls}`} style={{ fontSize: '0.65rem' }}>{d.severity}</span>}
                </div>
